<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-slate-900">{{ __('Edit Kategori') }}</h2>
    </x-slot>
            ...
            <div class="bg-white rounded-xl border border-slate-100 p-5 sm:p-6">
                <form action="{{ route('owner.categories.update', $category) }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <x-text-input id="name" name="name" type="text" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition" :value="old('name', $category->name)" required autofocus />
