            ...
            <div class="bg-white rounded-xl border border-slate-100 p-5 sm:p-6">
                <form action="{{ route('owner.products.store') }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <textarea id="description" name="description" rows="4" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition">{{ old('description') }}</textarea>
                        <x-input-error :messages="$errors->get('description')" class="mt-2" />
                    </div>

                    <div class="mb-5">
                        <x-input-label for="price" :value="__('Harga (Rp)')" class="block text-sm font-medium text-slate-700 mb-1" />
                        <x-text-input id="price" name="price" type="number" step="0.01" min="0" class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition" :value="old('price')" required />
